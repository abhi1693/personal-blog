...
function endActiveFaroUserAction() {
	...
	;(
		activeUserAction as
			| (typeof activeUserAction & { end?: () => void })
			| undefined
	)?.end?.()
}
...
async function getFaroRuntimeConfig(): Promise<FaroRuntimeConfig> {
	const buildTimeConfig: FaroRuntimeConfig = {
		...
		appName: process.env.NEXT_PUBLIC_FARO_APP_NAME || 'personal-blog',
	...
	try {
		...
		return {
			...
			appName: runtimeConfig.appName || 'personal-blog',
...
export default function FaroRum() {
	useEffect(() => {
		let cancelled = false
		...
		void getFaroRuntimeConfig().then((config) => {
			if (
				cancelled ||
				faroInitialized ||
				!config.enabled ||
